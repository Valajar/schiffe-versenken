...
export var leben = 3; // Startanzahl der Leben
...
export var highScore = localStorage.getItem('highScore');
...
export function gameStart() {
    ...
    window.updateLivesDisplay(leben); // Zeigt die verbleibenden Leben an
    window.updateTorpedoAvailability(); // Aktualisiert die Verfügbarkeit des Torpedos
}
...
window.onShoot = function(id, skipLifeCounter = false) {
    ...
    if (schiffe.includes(id)) {
        ...
        shots++;
        if (!skipLifeCounter) {
            lifeCounter(loseLifeChance);
        }
        if (treffer === numShips * schiffLength) {
            window.youWin(treffer, leben, diffMult, shots);
        }
    } else {
...
export function lifeCounter(loseLifeChance) {
    var randomNumber = Math.random();
    if (randomNumber < loseLifeChance) {
        leben--; 
        updateLivesDisplay(leben);
...
export function restartGame() {
    ...
    leben = 3; // Reset lives
    window.updateLivesDisplay(leben); // Update lives display
    window.updateScoreDisplay(); // Update score display
    window.updateTorpedoAvailability(torpedoShots); // Update torpedo display
    window.updateShotsDisplay(shots, treffer); // Update shots display
    gameStart();
}
